...
def xception():
    ...
    #Compiling model
    model = Model(inputs = tmodel_base.input, outputs = x, name = 'Our_Xception')
    model.summary()

    opt1 = Adam(lr=1e-5, beta_1=0.9, beta_2=0.999)
    xception_checkpoint = ModelCheckpoint("xception_freezed.h5", monitor='val_accuracy', verbose=1,
...
checkpoint = ModelCheckpoint('kmeans_data_augm_freezed_xception.h5', verbose=1, save_best_only=True)
...
# Last Model
model = load_model('kmeans_data_augm_freezed_xception.h5')
final_loss, final_accuracy = model.evaluate(X_val, Y_val)
print('Final Loss: {}, Final Accuracy: {}'.format(final_loss, final_accuracy))
